...
async def verify_captcha(app: App, given_solution: str, user_facing_message: str = "Success!"):
    """
    ...
    def decorator(func):
        ...
        async def decorated_function(request: Request, *args, **kwargs):
            ...
            if not isinstance(given_solution, str):
                return json({
                    "accept": False,
                    "message": "Captcha solution not present. Expected str, got " + type(given_solution).__name__
                }, status=400)
            ...
            if accept:
                response = await func(request, *args, **kwargs, user_facing_message=user_facing_message)
                return response
            else:
                return json({
                    "accept": False,
